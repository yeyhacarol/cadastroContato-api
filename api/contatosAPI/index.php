<?php
/* Os métodos de requisição para a API são:
* GET     -> para buscar dados;
* POST    -> para inserir novos registros;
* DELETE  -> para apagar registros;
* PUT     } 
            -> para editar dados, sendo o método put o mais utilizado         
* PATCH   }
*/
...
//EndPoint: requisição para alterar dados do contato. Estamos utilizando post ao invés de put porque o slim não trabalha com arquivos
$app->post('/contatos/{id}', function ($request, $response, $args) {
    //verificando se o que está no args é um número
    if (is_numeric($args['id'])) {
        $id = $args['id'];
        ...
        switch ($contentType[0]) {
            case 'multipart/form-data':
                ...
                //chamando função para buscar foto que já está salva no banco de dados 
                if ($dadosContato = buscarContato($id)) {
                    ...
                    if (is_bool($resposta) && $resposta == true) {
                        ...
                        return $response->withStatus(400)
                            ->withHeader('Content-Type', 'application/json')
                            ->write('{"message": "Registro não pôde ser editado.",             
                                        "erro": ' . $contatoJson . '}');
                    }
                } else {
                    return $response -> withStatus(404)
        ...
        return $response->withStatus(404)
            ->withHeader('Content-Type', 'application/json')
            ->write('{"message": "Obrigatório informar um ID válido(númerico)."}');
    }
});
